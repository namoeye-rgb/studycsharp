...
{
    public static class GoodsShop_Proc
    {
        public static ERROR_CODE CheckBuyItem(int _buyItemTID, out DummyData_ShopItem _shopListItem)
        {
            _shopListItem = ShopMgr.Instance.GetShopItem(SHOP_TYPE.GOODS, _buyItemTID);
            if (_shopListItem == null)
            {
                Log.Instance.Warn($"not found GoodsShop_Proc buyItemTID : {_buyItemTID}");
